...
from audioldm_train.utilities.tools import build_image_dataset_json_from_list
...
def infer(dataset_json, configs, config_yaml_path, exp_group_name, exp_name):
    ...
    aclp = AudioCLIP(pretrained=f'data/checkpoints/INaturalist_Multimodal-Audio-x2_ACLIP-CV1_ACLIP-CV1_performance=0.8012.pt')
    images_data = dataset_json["data"]
   
    xcl_metadata = pd.read_csv("data/dataset/metadata/xcl_v1/xcl_map_with_qids.csv")
    LABELS = xcl_metadata["display_name"].to_list()
    # print(LABELS)
    
    paths_to_images = glob.glob('tests/images/*.jpg')
    images = list()
    ...
    images = torch.stack([transforms.image_transforms(image) for image in images])
    ...
    for image_idx in range(len(paths_to_images)):
        ...
        conf_values, ids = confidence[image_idx].topk(1)
...
if __name__ == "__main__":
    ...
    dataset_json = build_image_dataset_json_from_list(args.list_inference)
